...
//				if (!bot.hasRemade) bot.updateRemake(false);
				...
				if (!bot.isHost) {
					...
					setTimeout(() => {
						console.log(bot.username + ' is joining ' + host.uid);
						bot.joinGame(host.uid, host.password);
					}, 2000);

				} else if (!bot.creatingGame) {
	...
	async function onToLobby(bot) {
		if (!bot.isHost) {
			...
			if ((!bot.game || !bot.gameState || bot.game.gameState.isCompleted) && !bot.creatingGame) createGame(bot);
